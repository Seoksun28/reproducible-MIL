...
# ===============================
# 데이터 & 환자 fold 준비 (seed와 무관, 한 번만)
# ===============================
def prepare_items_and_patients(case_dir, control_dir):
    items = load_items_from_dirs(case_dir, control_dir)
    ...
    return items, pid_to_indices, patients


# ===============================
# LOOCV (한 seed에 대해)
# ===============================
def run_loocv_for_seed(args, items, pid_to_indices, patients, result_dir, seed):
    os.makedirs(result_dir, exist_ok=True)

    # attention / vector 디렉토리
    attn_dir = os.path.join(result_dir, "attention")
    vec_dir = os.path.join(result_dir, "vector")
    ...
    with open(csv_path, "w", newline="") as csv_f:
        ...
        for fold_idx, test_pid in enumerate(patients):
            print(f"\n========== [seed {seed}] Fold {fold_idx+1}/{len(patients)} | Test PID = {test_pid} ==========")
            ...
            train_pids = [pid for pid in patients if pid != test_pid]
            config["folds"].append({
                "fold_index": fold_idx,
                "test_pid": test_pid,
            ...
            for epoch in range(1, args.epochs + 1):
                ...
                print(f"[seed {seed} | Fold {fold_idx+1}] Epoch {epoch}/{args.epochs} | Loss: {loss:.4f}")
    ...
    print("\n===== LOOCV DONE (seed={}) =====".format(seed))
    print(f"Overall ACC: {overall_acc:.4f}, AUROC: {overall_auroc:.4f}")
    print(f"Saved result.csv, result.txt, config.json under {result_dir}")
...
# ===============================
def parse_args():
    parser = argparse.ArgumentParser(description="Patient-wise LOOCV with TransMIL (5 seeds)")
    ...
    # etc
    parser.add_argument("--seed", type=int, default=42, help="(multi-seed에서는 무시됨)")
...
def main():
    ...
    # 1) 데이터 & 환자 fold는 한 번만 준비 (seed와 무관, 고정)
    items, pid_to_indices, patients = prepare_items_and_patients(
        args.case_dir, args.control_dir
    )
    ...
    for i, seed in enumerate(seeds, start=1):
        result_dir = os.path.join(base_result_dir, f"result{i}")
        print(f"\n========== Running LOOCV for seed={seed} -> {result_dir} ==========")
        run_loocv_for_seed(args, items, pid_to_indices, patients, result_dir, seed)
