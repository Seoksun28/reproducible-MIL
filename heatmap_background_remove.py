...
from PIL import Image  # RGBA → 흰 배경 합성용
...
def get_clam_style_heatmap(slide_path: str,
    ...
    # 11) 조직이 있는 위치 (패치가 실제로 있었던 위치)
    tissue_mask = (count_mask > 0)  # bool mask (H, W)

    # 12) 원본 이미지 읽기 (RGBA → 흰 배경 합성 후 BGR)
    print("Overlaying on WSI...")

    region = slide.read_region((0, 0), vis_level, (w_vis, h_vis))  # 보통 RGBA
    if region.mode == "RGBA":
        bg = Image.new("RGBA", region.size, (255, 255, 255, 255))  # 흰 배경
        region = Image.alpha_composite(bg, region)

    original_img = np.array(region.convert("RGB"))
    original_img = cv2.cvtColor(original_img, cv2.COLOR_RGB2BGR)

    # 13) 기본 이미지는 무조건 원본
    overlay = original_img.copy()

    # 14) blended 이미지는 전체 한 번 계산
    blended = cv2.addWeighted(original_img, 1 - ALPHA, heatmap_color, ALPHA, 0)

    # 15) 패치가 실제로 존재하는 위치(tissue_mask==True)에만 overlay 적용
    #     → (y, x) 단위 boolean mask를 픽셀 단위 인덱스로 사용
    overlay[tissue_mask] = blended[tissue_mask]
